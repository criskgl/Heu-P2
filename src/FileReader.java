		...
		for(int i = 0; i<st; i++) {
			for(int j = 0; j<pl; j++) {
				if(text.charAt(cont) == '_')
					isEmpty[i][j] = new BooleanVar(store, "La posici�n "+j+" de la calle "+i+" est�: Vac�a"); 
				else
					isEmpty[i][j] = new BooleanVar(store, "La posici�n "+j+" de la calle "+i+" est�: Ocupada"); 
				cont+=2;
		...
		for(int i = 0; i < st; i++) {
			for(int j = 0; i < pl; j++) {
				if(text.charAt(cont)<text.charAt(cont+2))
					catSup[i][j] = new BooleanVar(store, "La categor�a de la posici�n "+j+" de la calle "+i+" NO es mayor que la posici�n de la derecha"); 
				else
					catSup[i][j] = new BooleanVar(store, "La categor�a de la posici�n "+j+" de la calle "+i+" SI es mayor que la posici�n de la derecha"); 
